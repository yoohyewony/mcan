...
class GradReverseMask(torch.autograd.Function):
    ...
    def forward(ctx, x, mask, weight):
        ...
        ctx.weight = weight
        return x.view_as(x)    # view(x.size())
    ...
    def backward(ctx, grad_output):
        ...
        return grad_output * mask_c[:, None].float(), None, None    # None creates axis with length 1
...
class GradReverse(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        return grad_output.neg()    # .neg(): returns a new tensor with the negative of the elements of input
