...
def shuffle_list(ans_list):
    random.shuffle(ans_list)    # takes sequence (list, string, tuple) and reorganizes the order of the item
...
def img_feat_path_load(path_list):
    ...
    for ix, path in enumerate(path_list):
        coco_split = str(path.split('/')[-1].split('_')[-2])
        iid = str(int(path.split('/')[-1].split('_')[-1].split('.')[0]))
        iid = coco_split + '_' +str(iid)
...
def proc_img_feat(img_feat, img_feat_pad_size):
    ...
    img_feat = np.pad(
        img_feat,
        ((0, img_feat_pad_size - img_feat.shape[0]), (0, 0)),    # right side zero-padding
        mode='constant',
        constant_values=0
    )
